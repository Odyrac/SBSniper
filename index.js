...
client.on("ready", async () => {
  ...
  const channel = client.channels.cache.get('821359237993070652');
  const message = await channel.messages.fetch('828671771418034187');
  snipe(message.channel);

  async function snipe(channel, message) {
  
    let apiah = `https://api.hypixel.net/skyblock/auctions?key=${process.env.apikey}&page=`;
  ...
  function annonceitem(item_name, item_price, maxprixitem, channel, uuid) {
    if (item_name in maxprixitem) {
      ...
      .setFooter("• Odyrac")
      channel.send(`ping`) //@everyone or <@&ID-OF-ROLE>
      channel.send(embed)
    }
  }
