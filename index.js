...
(async () => {
  await client.login('TOKEN-BOT'); //change here
})();

client.on("ready", async () => {
  console.log(`${client.user.tag} online!`);
  client.user.setActivity('Sniper AH');
  const channel = client.channels.cache.get('ID-CHANNEL'); //change here
  const message = await channel.messages.fetch('ID-MESSAGE'); //change here
  ...
  async function snipe(channel, message) {
    ...
    while (true) {
      let apireponse = await fetch(apiah + "1");
      if (apireponse.ok) {
        const json = await apireponse.json();
        let maxpage = json.maxpage
